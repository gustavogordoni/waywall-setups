...
local show_mirrors = function(f3, tall, thin, wide)
    ...
    if cfg.thin_pie.enabled then
        if cfg.thin_pie.colorkey then
            mirrors.thin_pie_entities(tall)
            mirrors.thin_pie_unspecified(tall)
            mirrors.thin_pie_blockentities(tall)
            mirrors.thin_pie_destroyProgress(tall)
            mirrors.thin_pie_prepare(tall)
        else
...
local resolutions = {
    thin = make_res(384, cfg.res_1440 and 1100 or 1080, thin_enable, res_disable),
...
config.actions = {
    ...
    [cfg.thin.key] = resize_helper(cfg.thin, function()
        resolutions.thin()
    end),
    [cfg.wide.key] = resize_helper(cfg.wide, function()
        resolutions.wide()
    end),
    [cfg.tall.key] = resize_helper(cfg.tall, function()
        resolutions.tall()
    end),
    ...
    [cfg.toggle_remaps_key] = function()
        ...
        if remaps_active then
            ...
            if cfg.remaps_config.enabled then
                waywall.set_keymap({
                    layout = "br"
                })
            end
            ...
            if cfg.remaps_config.enabled then
                waywall.set_keymap({
                    layout = cfg.remaps_config.layout_name
                })
            end
        end
    end
