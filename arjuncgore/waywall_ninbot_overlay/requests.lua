...
-- replace: local xml = require('xml')
local xml
do
  local ok, mod = pcall(require, "xml")
  if ok and type(mod) == "table" and type(mod.load) == "function" then
    xml = mod
  else
    xml = nil  -- no XML parser; we'll skip response.xml
  end
end
...
-- lazy-load md5 only if needed (Digest Auth)
local md5sum
local function ensure_md5()
  if not md5sum then
    local ok, mod = pcall(require, "md5")
    if ok then
      md5sum = mod
    else
      error("Digest Auth requires the 'md5' module. Install with: luarocks --lua-version=5.1 install md5")
    end
  end
end
